...
- **What** you are refactoring (specific files, functions, components).
- **Why** (performance, readability, removing duplication, reducing coupling).
- **What you are NOT changing** (behavior, API contracts, database schema).
...
|---|---|---|
| Extract service | Logic in route handler | Logic in `services/<resource>.py` |
| Extract CRUD | Raw queries in services | Queries in `crud/<resource>.py` |
| Add mixin | Repeated model columns | Shared mixin in `models/mixins.py` |
| Config cleanup | Scattered `os.getenv()` | Centralized `core/config.py` |

#### Frontend
| Pattern | Before | After |
|---|---|---|
| Extract hook | Logic in component | Custom hook in `hooks/` |
| Extract component | Large component file | Smaller focused components |
| Consolidate types | Inline types everywhere | Shared types in `types/` |
| Centralize API | `fetch()` in components | API module in `api/` |
