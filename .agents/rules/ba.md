...
- Attach wireframes or mockups when the story involves UI changes.
...
5. **Data Requirements** — new fields, schema changes, migration needs.
6. **API Contract** — endpoint path, method, request/response shapes (align with OpenAPI spec).
...
- Before development begins, BA must review the proposed API contract (auto-generated OpenAPI spec or hand-drafted).
- Validate that field names match the data dictionary.
- Confirm request/response shapes satisfy all frontend requirements.
